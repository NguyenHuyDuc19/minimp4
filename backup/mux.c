...
int main(int argc, char **argv)
{
    ...
    int is_hevc = (0 != strstr(argv[i], "265")) || (0 != strstr(argv[i], "hevc")); // if using h264, is_hevc will be zero
    ...
    ssize_t pcm_size;
    int16_t *alloc_pcm;
    int16_t *buf_pcm = alloc_pcm = (int16_t *)preload("raw_audio.pcm", &pcm_size);
    if (!buf_pcm)
    {
